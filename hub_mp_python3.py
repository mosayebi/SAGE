...
def get_helix_atom_ids(mol_id):
  ...
  if (mol_id%2 == 0):
      atom_id = list(range(atom1, atom1+3))
  else:
      atom_id = list(range(atom1+10, atom1+13)) 
  return atom_id
...
def get_patch_atom_ids(mol_id):
  ...
  atom1 = int(mol_id/2)*16
  if (mol_id%2 == 0):
      atom_id = [list(range(atom1+3, atom1+6)), list(range(atom1+6, atom1+9))]
      #atom_id = [atom1+3, atom1+6, atom1+4, atom1+7, atom1+5, atom1+8]
  else:
      atom_id = [list(range(atom1+13, atom1+16))] 
  return atom_id
...
def plot_core_shell_form_factor():
    ...
    f = [core_shell_form_factor(x, Rc, Rs) for x in s]
